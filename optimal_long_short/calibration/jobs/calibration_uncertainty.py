...
contiguous blocks of the causal-EWM residual increments; recalibrates only the
...
from optimal_long_short.calibration.calibration_uncertainty import (
    PARAMETER_NAMES,
...
from optimal_long_short.calibration.jobs.calibrate_eth_btc import (
    ...
    prepare_ewm_residual_increments,
)
...
from optimal_long_short.model.drift_service import with_expected_log_return_drift
...
def _replicate_worker(task: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # Preserve the point procedure: resample the direct EWM residuals
        # without imposing an additional sample-mean subtraction.
        sample1 = r1[indices]
        sample2 = r2[indices]
...
def main() -> None:
    ...
    if preprocessing_meta.get("method") != (
        "lagged_normalized_ewm_residual_increments"
    ):
        raise ValueError(
            "The empirical artifact does not use direct causal-EWM residual increments"
        )
    ...
    r1, r2, preprocessing = prepare_ewm_residual_increments(
        r_weth_raw,
        r_wbtc_raw,
        dt_years=dt_years,
    ...
    if len(r1) != int(meta.get("n_obs", len(r1))):
        raise ValueError("The residual-increment sample length differs from the point fit")
    for asset in market_order:
        reproduced_trend = float(
            preprocessing[asset].ewm_mean_path_per_period[-1] / dt_years
        )
        if not math.isclose(
            reproduced_trend,
    ...
    metadata = {
        ...
        "interpretation": (
            ...
            "criterion, direct causal-EWM residual preprocessing, point-selected asset "

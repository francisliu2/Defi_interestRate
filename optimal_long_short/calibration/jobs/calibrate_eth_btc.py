...
The ECF fit estimates only diffusion, jump, and dependence shape from residual
increments obtained by subtracting the lagged causal EWM mean once. The
residual log means are fixed at zero. Endpoint EWM
...
Usage :  python jobs/calibration/calibrate_eth_btc.py
...
def prepare_ewm_residual_increments(
    r_eth: np.ndarray,
    r_btc: np.ndarray,
    *,
    dt_years: float,
    horizon_years: float = EMPIRICAL_HORIZON_YEARS,
) -> tuple[np.ndarray, np.ndarray, dict[str, EWMResidualResult | float]]:
    """Return direct causal-EWM residual increments and preprocessing metadata."""
    ...
    eth = construct_ewm_residual_increments(r_eth, half_life_periods)
    btc = construct_ewm_residual_increments(r_btc, half_life_periods)
    if len(eth.residual_increments) != len(btc.residual_increments):
        raise ValueError("Causal WETH/WBTC residual increments are not aligned")
    return (
        eth.residual_increments,
        btc.residual_increments,
        {
            "WETH": eth,
            "WBTC": btc,
...
def compute_avg_rates(df: pd.DataFrame) -> dict[str, float]:
    """
    Average annualized AAVE APRs and return annual decimal rates.

    The history collector stores AAVE's RAY-encoded annual rates as percentage
    points (for example, 2.8 means 2.8% APR). Dividing by 100 produces a
    year^{-1} decimal rate (0.028) on the same annual time scale as the EWM
    drift and Kou parameters. No observation-frequency scaling belongs here.
    """
    columns = {
        "supply_btc": "supply_apr_btc",
        "borrow_eth": "variable_borrow_apr_eth",
        "supply_eth": "supply_apr_eth",
        "borrow_btc": "variable_borrow_apr_btc",
    }
    rates: dict[str, float] = {}
    for name, column in columns.items():
        values = df[column].to_numpy(dtype=float)
        if values.size == 0 or not np.all(np.isfinite(values)):
            raise ValueError(f"{column} must contain finite annual APR percentages")
        if np.any(values < 0.0):
            raise ValueError(f"{column} must contain non-negative annual APR percentages")
        rates[name] = float(np.mean(values)) / 100.0
    return rates
...
def empirical_asset_log_means(
    annualized_ewm_log_drift: dict[str, float],
) -> dict[str, float]:
    """Carry-free expected log-return means used to select the legs."""
    return {
        "WETH": float(annualized_ewm_log_drift["WETH"]),
        "WBTC": float(annualized_ewm_log_drift["WBTC"]),
    }


def select_long_short(
    log_mean_pre_carry: dict[str, float],
    rates: dict[str, float],
) -> tuple[str, str, dict[str, dict[str, float | str]]]:
    """Choose the role assignment with the largest carry-adjusted mu spread."""
    required = {"WETH", "WBTC"}
    if set(log_mean_pre_carry) != required:
    ...
    for long_asset in sorted(required):
        ...
        g_long = float(log_mean_pre_carry[long_asset] + supply_rate)
        g_short = float(log_mean_pre_carry[short_asset] + borrow_rate)
        candidates[long_asset] = {
            ...
            "g_long": g_long,
            "g_short": g_short,
            "g_spread": g_long - g_short,
        }
    selected = max(candidates.values(), key=lambda row: float(row["g_spread"]))
    if float(selected["g_spread"]) <= 0.0:
        raise ValueError("Neither role assignment has a positive expected-log-mean spread")
...
def build_empirical_params(
    ...
    pre_carry = empirical_asset_log_means(annualized_ewm_log_drift)
    ...
    final_log_mean1, final_log_mean2 = expected_log_return_drift(final_params)
    if final_log_mean1 <= final_log_mean2:
        raise ValueError(
            "The selected long leg does not have the larger expected log-return mean "
            "after role-specific carry"
        )
    selection = {
        "rule": "maximize the positive role-adjusted expected-log-return-mean spread",
        "long_asset": long_asset,
        "short_asset": short_asset,
        "log_mean_pre_carry": pre_carry,
        "candidate_assignments": candidates,
        "long_supply_rate": supply_rate,
        "short_borrow_rate": borrow_rate,
        "final_log_mean1": final_log_mean1,
        "final_log_mean2": final_log_mean2,
...
def print_summary(
    ...
    print(
        f"Raw increments={meta['n_raw_returns']}, causal EWM residuals={result.n_obs} "
    ...
    pre_carry = selection["log_mean_pre_carry"]
    for asset in (long_asset, short_asset):
        ...
        final_log_mean = float(pre_carry[asset]) + role_rate
        role = "long/supply" if asset == long_asset else "short/borrow"
        print(
            f"  {asset} ({role}): EWM log drift {trends[asset]:+.6f} "
            f"+ carry {role_rate:+.6f} = log mean {final_log_mean:+.6f}; "
            f"internal price-growth correction {residual_mu:+.6f} "
            f"gives mu {final_mu:+.6f}"
        )
...
def plot_ecf_fit(
    ...
    title = (
        rf"Shape-only ECF fit — WETH/WBTC EWM residuals $(s,-s)$, {frequency} data"
...
def save_params(
    ...
    drift_components = {
        long_asset: {
            ...
            "log_mean_pre_carry": selection["log_mean_pre_carry"][long_asset],
            "role_carry": selection["long_supply_rate"],
            "final_mu_price_growth": final_params.mu1,
            "final_expected_log_return_drift": final_log1,
            "mean_jump_rate": final_params.mean_jump_rate1,
            "log_sde_coefficient_muX": final_params.muX1,
        },
        short_asset: {
            "role": "short_debt",
            "ewm_expected_log_drift": trends[short_asset],
            "residual_price_growth_correction": residual_oriented.mu2,
            "log_mean_pre_carry": selection["log_mean_pre_carry"][short_asset],
            "role_carry": selection["short_borrow_rate"],
            "final_mu_price_growth": final_params.mu2,
            "final_expected_log_return_drift": final_log2,
            "mean_jump_rate": final_params.mean_jump_rate2,
            "log_sde_coefficient_muX": final_params.muX2,
        },
    }
    payload = {
        "_note": (
            "Generated by jobs/calibration/calibrate_eth_btc.py. "
        ...
        "meta": {
            ...
            "returns_preprocessing": {
                "method": "lagged_normalized_ewm_residual_increments",
                "causal": True,
                ...
                "residual_sample_mean_per_period": meta["residual_sample_mean_per_period"],
                "additional_sample_demeaning": False,
            },
            "drift_convention": {
                "params_mu": (
                ...
                "construction": (
                    "g_i = endpoint EWM expected-log drift + role-specific carry; "
                    "the stored price-growth exponent is mu_i = g_i + "
                    "0.5*sigma_i^2 + lambda_i*(chi_i-E[J_i]); the log-SDE "
                    "coefficient is muX_i = g_i-lambda_i*E[J_i]"
                ),
            ...
            "aave_rate_aggregation": {
                ...
                "date_range": [date0, date1],
                "source_units": "annual APR percentage points from AAVE RAY rates",
                "conversion": "divide percentage points by 100; no sampling-frequency scaling",
                "model_units": "annual decimal rate (year^-1)",
                "drift_treatment": (
                    "treated as annualized instantaneous carry and added to the "
                    "annualized expected log-return mean"
                ),
            },
            "source_files": {
                "WBTC": meta["btc_file"],
...
def main() -> None:
    ...
    r_eth, r_btc, preprocessing = prepare_ewm_residual_increments(
        r_eth_raw,
        r_btc_raw,
        dt_years=meta["dt_years"],
    )
    ...
    assert isinstance(eth_pre, EWMResidualResult)
    assert isinstance(btc_pre, EWMResidualResult)
    meta.update(
        {
            ...
            "endpoint_ewm_mean_per_period": {
                "WETH": float(eth_pre.ewm_mean_path_per_period[-1]),
                "WBTC": float(btc_pre.ewm_mean_path_per_period[-1]),
            },
            "annualized_ewm_log_drift": {
                "WETH": float(eth_pre.ewm_mean_path_per_period[-1] / meta["dt_years"]),
                "WBTC": float(btc_pre.ewm_mean_path_per_period[-1] / meta["dt_years"]),
            },
            "residual_sample_mean_per_period": {
                "WETH": eth_pre.residual_sample_mean,
                "WBTC": btc_pre.residual_sample_mean,
            },
        }
    )
    ...
    print(
        f"N={len(r_eth_raw)} raw increment pairs and {len(r_eth)} causal EWM "
        f"residual pairs ({date0} → {date1})"
    )
    print(
        "Subtracted the lagged normalized causal EWM mean once, with half-life "

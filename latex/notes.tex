...
Leverage & Optional, moderate & Alpha-scaled & Allocation $r$, $\beta_P \approx 0$ & Borrow fraction $\varphi$, repeated $k$ loops \\ \hline

Target Return & Yield spread long/short & Trending returns & $R_P = w_L \alpha_L - w_S \alpha_S$ & $r_{\mathrm{net}}^{(\infty)} = \frac{r_A - \varphi r_b}{1-\varphi}$ \\ \hline
...
Log-Health Process & $h(t)=\log\frac{\text{long}}{\text{short}}$ & $h(t)$ buffer & $h(t)=\log\frac{b_L w_L L_0}{w_S S_0}+(\log L-\log S)$ & $h_t=\log H_t - \log D_t$ \\ \hline

Health Exponent & $\psi_h \approx \psi_{\text{long}}+\psi_{\text{short}}(-)$ & Momentum asset dynamics & $\psi_h = \psi_L + \psi_S(-)$ & $\psi_h = \psi_X + \psi_Y(-)$ \\ \hline

Drift & Mean yield diff & Expected alpha & $\kappa_h=\mathbb{E}[h(t)]/t$ & $r_A/(1-\varphi) - r_b \varphi/(1-\varphi) - \mu_Y$ \\ \hline

Volatility / Jump & $\sigma_{\text{long}},\sigma_{\text{short}}$ & Asset vol, jumps & $\sigma_L,\sigma_S,\lambda_L,\lambda_S$ & $\sigma_Y,\lambda_Y,\delta_Y$ \\ \hline

Terminal Wealth & $W_T=W_0 + R_{\text{long}} - R_{\text{short}}$ & $W_T$ from momentum & $W_T=W_0 + w_L R_L - w_S R_S$ & $W_T=H_t - D_t$ \\ \hline

Liquidation & $h_t\le0$ & Margin call risk & $h_0(r)$ via $r,b_L$ & $h_t\le-\ell$ \\ \hline

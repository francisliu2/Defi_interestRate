...
Given the nature of stablecoins, their price movements can be assumed to be almost null: ($\psi_X(1) \approx \psi_X(2) \approx 0$):
\begin{align}
...
\begin{equation}
h(t) = \log\!\left(\frac{b_L w_L L_0}{w_S S_0}\right) + \big(\log L(t) - \log S(t)\big),
\end{equation}
where $L(t)$ and $S(t)$ are the long and short asset price processes, $L_0, S_0$ their initial prices, and $b_L$ represents the collateral factor.
...
\begin{equation}
W_T = W_0 + w_L R_L(T) - w_S R_S(T),
\end{equation}
where $R_L(T) = \frac{L_T}{L_0}-1$ and $R_S(T) = \frac{S_T}{S_0}-1$ are the respective returns of the long and short legs.

\paragraph{Expected Wealth and Variance of Wealth.}
The first two moments of the wealth process are then:
\begin{align}
\mathbb{E}[W_T] &= W_0 + w_L \big(e^{T \psi_L(1)} - 1\big) - w_S \big(e^{T \psi_S(1)} - 1\big), \\
...
\begin{align}
\text{Var}[R_i(T)] &= e^{T \psi_i(2)} - e^{2T \psi_i(1)}, \qquad i \in \{L,S\}.
\end{align}
...
\end{equation}
with $r = \frac{w_L}{w_S}$ the relative position ratio and $W_T(r)$ the terminal wealth at $T^*$. The initial log-health is
\begin{equation}
...
\begin{equation}
G(r) = \mathbb{E}\!\left[ \log\!\left(1 + f (w_L R_L(T^*) - w_S R_S(T^*)) \right) \right].
\end{equation}
...
\begin{enumerate}
    \item When the drift differential $(\mu_L - \mu_S)$ is large, $r^*$ increases almost linearly with the momentum strength.
    \item Higher jump intensity $\lambda_S$ and volatility $\sigma_S$ reduce $r^*$, as downside risk of the short leg dominates.
...
\end{equation}
with $L(t)$, $S(t)$ the long and short price processes and $b_L$ is the collateral factor. The Laplace exponent of $h(t)$ is
\begin{equation}
\psi_h(\theta) = \psi_L(\theta) + \psi_S(-\theta),
\end{equation}

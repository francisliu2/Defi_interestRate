...
We start with an initial USDC collateral \(C_0>0\). Using the protocol's collateral factor (maximum borrow fraction) \(\varphi\in(0,1)\), we borrow BTC up to a USD-valued amount \(\varphi C_0\) (valued at spot price \(S_t\) USDC/BTC). We then immediately swap the borrowed BTC into USDC and deposit the proceeds back into AAVE, which yields a supply rate \(r_A\). We can repeat the borrow–swap–deposit loop \(k\) times to create looped leverage; 
when \(k\to\infty\) and the protocol allows, the deposit converges to a geometric-series multiple of original collateral. The main risks of this strategy are: (i) the borrow interest on the BTC debt (rate \(r_b\)), and (ii) the mark-to-market exposure to BTC price moves which can trigger liquidation.

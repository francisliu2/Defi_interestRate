...
\hline
\textbf{Feature / Parameter} & \textbf{Stablecoin Long–Short Volatile} & \textbf{Momentum Strategy} & \textbf{Beta-Neutral Long–Short} & \textbf{Leveraged Yield (USDC → BTC → USDC → AAVE)} \\ \hline

Collateral & Stablecoin & Crypto assets & Long/short crypto & USDC \\ \hline

Leverage & Optional, moderate & Position size scaling by alpha signals & Allocation ratio $r$, maintains $\beta_P \approx 0$ & Borrow fraction $\varphi$ per loop, repeated $k$ times \\ \hline

Target Return & Earn yield spread between long and short stablecoins & Capture trending returns of selected assets & $R_P = w_L \alpha_L - w_S \alpha_S$ & $r_{\mathrm{net}}^{(\infty)} = \frac{r_A - \varphi r_b}{1-\varphi}$ \\ \hline

Main Risks & Asset volatility, short squeezes & Trend reversals, market shocks & Asset volatility, correlation breakdown, liquidation & BTC price moves, liquidation, fees/slippage \\ \hline

Log-Health Process & $h(t) = \log\frac{\text{long}}{\text{short}}$ & $h(t)$ linked to margin buffer & $h(t) = \log\frac{b_L w_L L_0}{w_S S_0} + (\log L(t) - \log S(t))$ & $h_t = \log H_t - \log D_t$ \\ \hline

Health Process Exponent & $\psi_h(\theta) \approx \psi_{\text{long}}(\theta) + \psi_{\text{short}}(-\theta)$ & $\psi_h(\theta)$ via momentum asset dynamics & $\psi_h(\theta) = \psi_L(\theta) + \psi_S(-\theta)$ & $\psi_h(\theta) = \psi_X(\theta) + \psi_Y(-\theta)$ \\ \hline

Drift & Mean yield difference & Expected alpha drift & $\kappa_h = \mathbb{E}[h(t)]/t$ & $r_A \frac{1}{1-\varphi} - r_b \frac{\varphi}{1-\varphi} - \mu_Y$ \\ \hline

Volatility / Jump Risk & $\sigma_{\text{long}}, \sigma_{\text{short}}$ & Asset volatility, jumps & $\sigma_L, \sigma_S, \lambda_L, \lambda_S$ & $\sigma_Y, \lambda_Y, \delta_Y$ \\ \hline

Terminal Wealth & $W_T = W_0 + R_{\text{long}} - R_{\text{short}}$ & $W_T$ based on momentum returns & $W_T = W_0 + w_L R_L(T) - w_S R_S(T)$ & $W_T = H_t - D_t$ \\ \hline

Liquidation Threshold & $h_t \le 0$ & Margin call risk from leveraged positions & $h_0(r)$ based on $r$ and $b_L$ & $h_t \le -\ell$ (protocol-defined) \\ \hline

Optimization & Max expected return / utility & Max Sharpe or alpha capture & Max utility or Kelly fraction $f^*(r)$ & Max net yield $r_{\mathrm{net}}(\varphi)$ subject to health drift $>0$ \\ \hline

Calibration & Historical stablecoin spreads & Historical momentum factor estimation & Rolling window $\mu_i, \sigma_i, \lambda_i, \delta_i$ & Historical BTC returns, $r_A, r_b$ from on-chain pools \\ \hline
\end{tabular}

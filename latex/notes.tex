...
This ensures unit net exposure while allowing flexibility in allocation between long and short positions, based on the relative expected returns and risk characteristics.
...
Collateral & Stablecoin & Long crypto & Long crypto & USDC \\ \hline

Leverage & None & Alpha-scaled & Depends on allocation $r$ & Up to $\frac{1}{1-\phi}$ (looped borrowing) \\ \hline

Target Return & Yield spread of long/short & Trending returns & $R_P = w_X \alpha_X - w_Y \alpha_Y$ & $r_{\mathrm{net}}^{(\infty)} = \frac{r_X - \varphi r_Y}{1-\varphi}$ \\ \hline
...
Log-Health Process & $\log \left( \frac{b_X \, w_X \, X(t)}{w_Y \, Y(t)} \right)$ & $\log \left( \frac{b_X \, w_X\, X(t)}{w_Y \, Y(t)} \right)$ & $\log \left( \frac{b_X \, w_X\, X(t)}{w_Y \, Y(t)} \right)$ & $\log \frac{(b_X w_X X(t))}{D_t}$ \\ \hline

Drift & $\kappa_h=\mu_X -\mu_Y$ & Can be simplified to $\mu_Y$ for risk-specific purposes & $\kappa_h=w_X \mu_X - w_Y \mu_Y$  & $r_X/(1-\varphi) - r_Y \varphi/(1-\varphi) - \mu_Y$ \\ \hline

Volatility / Jump & $\sigma_X,\sigma_Y, \lambda_Y$ & $\sigma_X,\sigma_Y,\lambda_X,\lambda_Y$ & $\sigma_X,\sigma_Y,\lambda_X,\lambda_Y$ & $\sigma_Y,\lambda_Y,\delta_Y$ \\ \hline

Terminal Wealth &  See \eqref{prop:Terminal Wealth Stablecoin}  & $W_T=W_0 + w_X R_X - w_Y R_Y$ & $W_T=W_0 + w_X R_X - w_Y R_Y$ & $W_T=H_t - D_t$ \\ \hline
...
Calibration & Stablecoin spreads & Momentum factor history & Rolling $\mu_i,\sigma_i,\lambda_i,\delta_i$ & BTC returns, $r_X,r_Y$ on-chain \\ \hline
\end{tabular}
...
\end{equation}
Given the nature of stablecoins, their price movements can be assumed to be almost null: ($\psi_X(1) \approx 0, \ \psi_X(2) \approx 0$):
\begin{align}
...
\end{equation}
where $W_T(r)$ follows the wealth process described above and the initial log-health is:
\begin{equation}
h_0(r) = \log \left( \frac{b_X w_X X_0}{w_Y Y_0} \right) 
= \log b_X + \log r + \log\!\left(\frac{X_0}{Y_0}\right).
\end{equation}
The trade-off between expected carry and probability of liquidation defines the region of optimal allocations.

\paragraph{Kelly Criterion Implementation.}
We use the Kelly criterion to determine optimal sizing, since our objective is to maximize the long-term growth rate of wealth. The core components ----win probability ($p$), loss probability ($q$), and net odds ($b$) ---- are derived from the jump-diffusion and liquidation framework.
...
\begin{align}
q &= \pi(T^*) = \mathcal{L}^{-1}\!\left[\frac{e^{-h_0 \Phi(q)}}{q}\right](T^*), \\
p &= 1-q, \\
...
The optimal Kelly fraction is defined as:
\begin{equation}
...
\end{equation}
For practical reasons, a fractional Kelly approach can be used:
\begin{equation}
...
\begin{enumerate}
    \item Compute $w_X(r)$, $w_Y(r)$, and the initial log-health $h_0(r)$.
    \item Evaluate the probability of liquidation $q(r) = \pi(T^*)$ via numerical Laplace inversion; set $p(r) = 1 - q(r)$.
    ...
    \item Optimize over $r$, ensuring practicality: $h_0(r) \ge h_{\min}$ and $0 < r < 1$.
\end{enumerate}

\paragraph{Calibration and DeFi Market Data.}
We calibrate the model using data from decentralized lending protocols and cryptocurrency markets obtained through Blockchain Research Center. Stablecoin supply rates $r_X$ typically range from 2--6\% per annum, while borrowing rates for volatile assets (e.g. ETH) range from 5--10\%.  
L\'evy jump–diffusion parameters $(\mu_i, \sigma_i, \lambda_i, \delta_i)$ are estimated using daily log returns. Stablecoins are considered to exhibit negligible diffusion and jump components ($\sigma_X \approx 0$, $\lambda_X \approx 0$), but volatile assets show significant jump intensities and fat tails.
...
\end{itemize}
Laplace inversions use the Gaver–Stehfest algorithm with $N=16$ terms (placeholder value).

\paragraph{Parameter Values - Placeholder.}
\begin{align*}
...
The second strategy we consider is a momentum strategy: here momentum is defined as performance over a specific period of time. A long position is taken in an asset with positive recent returns (``momentum winner''), and a short position is taken in an asset with negative recent returns (``momentum loser''). 
\\The idea underlying the strategy is fairly simple: the investor must buy tokens that are outperforming and short-sell those that are underperforming.
Given the nature of cryptocurrency markets, the challenge lies in identifying the proper horizon for the momentum signal and the optimal time to exit the positions. 
...
R(w_X, w_Y) = w_X R_X - w_Y R_Y,
\end{equation}
where $R_X$ is the expected return of the long asset, and $R_Y$ the expected return of the short asset.

\paragraph{Log-Health Process.}
In this setting, the log-health process tracks the relative value of the long and short positions:
\begin{equation}
h(t) = \log\!\left(\frac{b_X w_X X(t)}{w_Y Y (t)}\right)
\end{equation}
where $X(t)$ and $Y(t)$ are the long and short asset price processes, $X_0, Y_0$ their initial prices, and $b_X$ represents the collateral factor.

Assuming $\log X$ and $\log Y$ follow Lévy processes with Laplace exponents $\psi_X(\theta)$ and $\psi_Y(\theta)$, the Laplace exponent of $h(t)$ is:
\begin{equation}
\psi_h(\theta) = \psi_X(\theta) + \psi_Y(-\theta).
\end{equation}
In the case of correlated drivers with joint characteristic exponent ${\bf{\Psi}}_{X,Y}(\theta_1,\theta_2)$:
\begin{equation}
\psi_h(\theta) = {\bf{\Psi}}_{X,Y}(\theta,-\theta).
\end{equation}
...
\begin{equation}
W_T = W_0 + w_X R_X(t) - w_Y R_Y(t),
\end{equation}
where $R_X(t) = \frac{R_T}{X_0}-1$ and $R_Y(t) = \frac{Y_T}{Y_0}-1$ are the respective returns of the long and short legs.

\paragraph{Expected Wealth and Variance of Wealth.}
The first two moments of the wealth process are then:
\begin{align}
\mathbb{E}[W_T] &= W_0 + w_X \big(e^{T \psi_L(1)} - 1\big) - w_Y \big(e^{T \psi_S(1)} - 1\big), \\
\text{Var}(W_T) &= w_X^2 \text{Var}[R_X(t)] + w_Y^2 \text{Var}[R_Y(t)] - 2 w_X w_Y \text{Cov}[R_X(t),R_Y(t)].
\end{align}
Using the moment-generating property of Lévy processes, each component satisfies
\begin{align}
\text{Var}[R_i(T)] &= e^{T \psi_i(2)} - e^{2T \psi_i(1)}, \qquad i \in \{X,Y\}.
\end{align}

Thus, the expected return reflects the relative drift between the two assets, while the variance arises from their individual volatilities and correlation.

\paragraph{Optimization Problem.}
The investor’s mean–variance objective becomes:
\begin{equation}
\max_{r} \quad 
\mathcal{U}(r)
= \mathbb{E}[W_T(r)]
...
\end{equation}
with $r = \frac{w_X}{w_Y}$ the relative position ratio and $W_T(r)$ the terminal wealth at $T^*$. The initial log-health is
\begin{equation}
h_0(r) = \log\!\left( \frac{b_X w_X X_0}{w_Y Y_0} \right)
= \log b_X + \log r + \log\!\left(\frac{X_0}{Y_0}\right),
\end{equation}
which determines the effective margin requirement.
...
\paragraph{Kelly Criterion Implementation.}
We can also formulate the problem using the Kelly criterion, which maximizes the long-run geometric growth rate of wealth. 

\subparagraph{Expected Growth Rate.}
Under the assumption of independent Lévy increments, the log wealth growth over horizon $T^*$ is:
\begin{equation}
G(r) = \mathbb{E}\!\left[ \log\!\left(1 + f (w_X R_X(T^*) - w_Y R_Y(T^*)) \right) \right].
\end{equation}
Maximizing $G(r)$ with respect to the Kelly fraction $f$ yields the optimal allocation fraction:
\begin{equation}
f^*(r) = \frac{\mathbb{E}[A(r)]}{\mathbb{V}[A(r)]},
\end{equation}
where $A(r) = w_X R_X(T^*) - w_Y R_Y(T^*)$. 

For practical implementation, we use a fractional Kelly approach to reduce sensitivity to estimation errors:
\begin{equation}
f_{\text{target}}(r) = k \times f^*(r),
\end{equation}
where $k \in (0,1]$ (e.g., $k = 0.5$ for Half Kelly).

\paragraph{Calibration and Market Data.}
The model is calibrated on daily data for tokens with pronounced cross-sectional momentum. Momentum winners (long leg) and losers (short leg) are selected based on extreme cumulative returns over a $k=30$ day formation period. The Lévy jump-diffusion parameters $(\mu_i, \sigma_i, \lambda_i, \delta_i)$ for all assets are estimated through rolling windows of historical log-returns. Empirical evidence shows that momentum winners are characterized by a higher drift $\mu_X$ and comparable or slightly higher volatility $\sigma_X$ relative to losers. The jump parameters $(\lambda_i, \delta_i)$ effectively capture asymmetry and fat tails in crypto returns, reflecting the persistence and downside risk of trend reversals.

\paragraph{Numerical Evaluation.}
For each set of calibrated parameters, we compute $\mathcal{U}(r)$ and $f^*(r)$. Gradients are approximated numerically, and the optimal ratio $r^*$ is found using projected gradient ascent subject to feasibility constraints.

\paragraph{Parameter Values (Placeholder).}
\begin{align*}
\mu_X &= 0.10, &\sigma_X &= 0.80, &\lambda_X &= 0.10, &\delta_L &= 0.20,\\
\mu_Y &= 0.05, &\sigma_Y &= 0.70, &\lambda_Y &= 0.20, &\delta_S &= 0.25,\\
...
\begin{enumerate}
    \item When the drift differential $(\mu_X - \mu_Y)$ is large, $r^*$ increases almost linearly with the momentum strength.
    \item Higher jump intensity $\lambda_Y$ and volatility $\sigma_Y$ reduce $r^*$, as downside risk of the short leg dominates.
    \item Fractional Kelly allocations ($k<1$) provide smoother wealth trajectories with reduced variance.
\end{enumerate}


\subsection{Beta-Neutral Long--Short Strategy}

The third case we consider is a beta-neutral long-short strategy: this strategy aims to isolate idiosyncratic returns by neutralizing exposure to the overall cryptocurrency market. We take a long position in a high-alpha asset $\alpha_X$ and a short position in an asset with similar beta but lower alpha $\alpha_Y$, capturing the alpha spread 
while aiming to mitigate systematic risk.  

\paragraph{Portfolio Construction.}  
Let $\beta_i$ denote the market beta of asset $i$. Construct long leg ${X}$ and short leg ${Y}$ with weights $w_{x}, w_{y}$. Beta-neutrality requires
\[
\beta_P = \sum_{i \in {X}} w_i \beta_i - \sum_{j \in Y} w_j \beta_j \approx 0,
\]
...
\begin{equation}
\mathbb{E}[W_T] = W_0 + w_X(e^{T\psi_X(1)}-1) - w_Y(e^{T\psi_Y(1)}-1),
\end{equation}
and the variance is
\begin{equation}
\text{Var}[W_T] = w_X^2 \text{Var}[R_X(t)] + w_Y^2 \text{Var}[R_Y(t)] - 2 w_X w_Y \text{Cov}[R_X(t),R_Y(t)],
...
h(t) = \log\left(\frac{b_X w_X X(t)}{w_Y Y(t)}\right)
\end{equation}
with $X(t)$, $Y(t)$ the long and short price processes and $b_X$ is the collateral factor. The Laplace exponent of $h(t)$ is
\begin{equation}
\psi_h(\theta) = \psi_X(\theta) + \psi_Y(-\theta),
\end{equation}
...
\begin{itemize}
    \item \textbf{Loss Event:} liquidation occurs with probability $q = \pi(T^*) = \mathbb{P}(\tau_0 \le T^*)$. The net loss is $R_{\text{loss}} = -L w_X$.
    \item \textbf{Win Event:} no liquidation, probability $p = 1-q$. The gain is the expected idiosyncratic return $R_{\text{gain}} = w_X \alpha_X T^* - w_Y \alpha_Y T^*$.
\end{itemize}

The net odds are
\begin{equation}
b = \frac{R_{\text{gain}}}{|R_{\text{loss}}|} = \frac{w_X \alpha_X T^* - w_Y \alpha_Y T^*}{L w_X}.
\end{equation}
...
\begin{equation}
G(r) = q(r) \log(1 - f^*(r) L w_X) + p(r) \log(1 + f^*(r) b(r) L w_X).
\end{equation}

Once again, a fractional Kelly can be considered more practical $f_{\rm target}(r) = k f^*(r)$ (e.g., $k=0.5$).
\paragraph{Solution.}  
\begin{enumerate}
    \item For a given allocation ratio $r$, compute $w_X(r), w_Y(r)$ and initial log-health $h_0(r)$.
...
\paragraph{Parameters Values (Placeholder)}
\begin{align*}
\mu_X &= 0.08, &\sigma_X &= 0.75, &\lambda_X &= 0.08, &\delta_L &= 0.20,\\
\mu_Y &= 0.04, &\sigma_Y &= 0.70, &\lambda_Y &= 0.12, &\delta_S &= 0.22,\\
T^* &= 30 \text{ days}, &h_{\min} &= 0.1.
\end{align*}
...
Consider an initial USDC collateral \(C_0>0\). Using the protocol's collateral factor \(\varphi \in (0,1)\), BTC is borrowed up to a USD-valued amount \(\varphi C_0\). The borrowed BTC is immediately swapped into USDC and deposited back into AAVE at a supply rate \(r_X\). Repeating the borrow–swap–deposit loop \(k\) times generates a geometric series of leveraged deposits. The primary risks are the BTC borrow interest \(r_Y\) and mark-to-market exposure to BTC price movements, which can trigger liquidation.
...
\begin{itemize}
  ...
  \item \(r_X\) — AAVE supply APY for USDC.
  \item \(r_Y\) — borrow interest rate for BTC (USD-equivalent).
...
\[
r_{\mathrm{net}}^{(\infty)}(\varphi) = \frac{r_X - \varphi\, r_Y}{1-\varphi}.
\]
...
\[
\frac{r_X}{1-\varphi} - \frac{r_Y \varphi}{1-\varphi} - \mu_Y > 0,
\]
with \(\mu_Y = -\mathbb{E}[\dot Y_t]\) representing the effect of BTC price drift. Ignoring \(\mu_Y\) yields the heuristic constraint \(\varphi < r_X / r_Y\).
...
With \(r_X = 0.046\) and \(r_Y = 0.02\), the infinite-loop net yield is
\[
...
\label{alg:gaveR_Ytehfest_appendix}
\begin{algorithmic}[1]
...
The third case we consider is a beta-neutral long-short strategy: this strategy aims to isolate idiosyncratic returns by neutralizing exposure to the aggregate cryptocurrency market. The portfolio takes a long position in a high-alpha asset $\alpha_X$ and a short position in an asset with similar beta but lower alpha $\alpha_Y$, capturing the alpha spread while mitigating systematic risk.  
...
Ignoring liquidation events, the expected terminal wealth is
\begin{equation}
\mathbb{E}[W_T] = W_0 + w_X(e^{T\psi_L(1)}-1) - w_Y(e^{T\psi_S(1)}-1),
\end{equation}
and the variance is
\begin{equation}
\text{Var}[W_T] = w_X^2 \text{Var}[R_X(t)] + w_Y^2 \text{Var}[R_Y(t)] - 2 w_X w_Y \text{Cov}[R_X(t),R_Y(t)],
...
Define
\begin{equation}
h(t) = \log\left(\frac{b_X w_X X_0}{w_Y Y_0}\right) + (\log X(t) - \log Y(t)),
\end{equation}
with $X(t)$, $Y(t)$ the long and short price processes and $b_X$ a collateral factor. The Laplace exponent of $h(t)$ is
\begin{equation}
